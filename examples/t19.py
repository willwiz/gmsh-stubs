...
gmsh.model.occ.add_circle(-0.1, -0.1, 2, 0.3, 3)
gmsh.model.occ.add_curve_loop([3], 3)
gmsh.model.occ.add_thru_sections([1, 2, 3], 1)
gmsh.model.occ.synchronize()

# We can also force the creation of ruled surfaces:
gmsh.model.occ.add_circle(2 + 0, 0, 0, 0.5, 11)
gmsh.model.occ.add_curve_loop([11], 11)
gmsh.model.occ.add_circle(2 + 0.1, 0.05, 1, 0.1, 12)
gmsh.model.occ.add_curve_loop([12], 12)
gmsh.model.occ.add_circle(2 - 0.1, -0.1, 2, 0.3, 13)
gmsh.model.occ.add_curve_loop([13], 13)
gmsh.model.occ.add_thru_sections([11, 12, 13], 11, makeSolid=True, makeRuled=True)
gmsh.model.occ.synchronize()
...
gmsh.model.occ.synchronize()
e = gmsh.model.get_boundary(gmsh.model.get_boundary(out), combined=False)
...
for i in range(npts):
    ...
    gmsh.model.occ.add_point(r * math.cos(theta), r * math.sin(theta), i * h / npts, 1, 1000 + i)
    p.append(1000 + i)
gmsh.model.occ.add_spline(p, 1000)

# A wire is like a curve loop, but open:
gmsh.model.occ.add_wire([1000], 1000)

# We define the shape we would like to extrude along the spline (a disk):
gmsh.model.occ.add_disk(1, 0, 0, 0.2, 0.2, 1000)
...
gmsh.option.set_number("Mesh.MeshSizeMin", 0.001)
gmsh.option.set_number("Mesh.MeshSizeMax", 0.3)
