...
gmsh.model.geo.add_point(1, 1, 0.5, lc, 3)
gmsh.model.geo.add_point(0, 1, 0.4, lc, 4)
gmsh.model.geo.add_point(0.3, 0.2, 0, lc, 5)
gmsh.model.geo.add_point(0, 0.01, 0.01, lc, 6)
gmsh.model.geo.add_point(0, 0.02, 0.02, lc, 7)
gmsh.model.geo.add_point(1, 0.05, 0.02, lc, 8)
gmsh.model.geo.add_point(1, 0.32, 0.02, lc, 9)

gmsh.model.geo.add_line(1, 2, 1)
gmsh.model.geo.add_line(2, 8, 2)
gmsh.model.geo.add_line(8, 9, 3)
gmsh.model.geo.add_line(9, 3, 4)
gmsh.model.geo.add_line(3, 4, 5)
gmsh.model.geo.add_line(4, 7, 6)
gmsh.model.geo.add_line(7, 6, 7)
gmsh.model.geo.add_line(6, 1, 8)
gmsh.model.geo.add_spline([7, 5, 9], 9)
gmsh.model.geo.add_line(6, 8, 10)

gmsh.model.geo.add_curve_loop([5, 6, 9, 4], 11)
gmsh.model.geo.add_surface_filling([11], 1)

gmsh.model.geo.add_curve_loop([-9, 3, 10, 7], 13)
gmsh.model.geo.add_surface_filling([13], 5)

gmsh.model.geo.add_curve_loop([-10, 2, 1, 8], 15)
gmsh.model.geo.add_surface_filling([15], 10)

gmsh.model.geo.synchronize()

# Treat curves 2, 3 and 4 as a single curve when meshing (i.e. mesh across
# points 6 and 7)
gmsh.model.mesh.set_compound(1, [2, 3, 4])

# Idem with curves 6, 7 and 8
gmsh.model.mesh.set_compound(1, [6, 7, 8])

# Treat surfaces 1, 5 and 10 as a single surface when meshing (i.e. mesh across
# curves 9 and 10)
gmsh.model.mesh.set_compound(2, [1, 5, 10])
